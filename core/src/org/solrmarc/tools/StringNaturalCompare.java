...
public class StringNaturalCompare implements Comparator<String>
{
   ...
   remember it in BIAS. */
    ...
    public static int strnatcmp0(String s1, String s2, boolean fold_case)
    {
        ...
        while (true)
        {
            char ca = ai < s1len ? s1.charAt(ai) : 0;
            char cb = bi < s2len ? s2.charAt(bi) : 0;

            /* skip over leading spaces or zeros */
            while (Character.isWhitespace(ca))
            {
                ai = ai + 1;
                ca = ai < s1len ? s1.charAt(ai) : 0;
            }

            while (Character.isWhitespace(cb))
            {
                bi = bi + 1;
                cb = bi < s2len ? s2.charAt(bi) : 0;
            }

            /* process run of digits */
            if (Character.isDigit(ca) && Character.isDigit(cb))
            {
                boolean fractional = (ca == '0' || cb == '0');

                if (fractional)
                {
                    if ((result = compareLeft(s1, ai, s2, bi)) != 0)
                    	return result;
                }
                else
                {
                    if ((result = compareRight(s1, ai, s2, bi)) != 0)
                    	return result;
                }
            }
